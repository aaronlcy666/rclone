...
var commandDefinition = &cobra.Command{
	...
	Long: `
This runs a backend specific command. The commands themselves are
defined by the backends and you should see the backend docs for
definitions.

You can discover what commands a backend understands by using
...
Pass options to the backend command with -o. This should be key=value or key, eg:
    ...
    rclone backend remote:path stats -o format=json -o long
...
`,
	RunE: func(command *cobra.Command, args []string) error {
		...
		cmd.Run(false, false, command, func() error {
			...
			// show help if remote is a remote
			if name == "help" {
				return showHelp(fsInfo)
			}
			// Get the command
